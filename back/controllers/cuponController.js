...
const registro_cupon_admin = async function(rq,res){
if (req.user) {
...
const obtener_cupon_admin = async function(req,res){
    if (req.user) {
        if (req.user.role =='administrador') {
          ...
          try {
            var rag = await Cupon.findById({_id:id});
            res.status(200).send({data:reg});
          } catch (error) {
...
const actualizar_cupon_admon = async function(req,res){
    if (req.user) {
        if (req.user.role =='administrador') {
